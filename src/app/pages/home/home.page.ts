...
export class HomePage implements OnInit {
    ...
    if (user) {
      ...
      this.sesionesSubscription = this.sesionesSubscriptionService.subscribe().subscribe((change: CollectionChange<any>) => {
        if (change.type === 'added') {
          const yaExiste = this.sesiones.find(s => s.id === change.id);
          if (!yaExiste) {
  ...
  ngOnDestroy() {
    ...
    this.sesionesSubscriptionService.unsubscribe(); // ✅ Cerrar la suscripción a Firestore
  }
  ...
  async openCreateSesionModal() {
    ...
    // ❌ Ya no hacemos push manualmente
